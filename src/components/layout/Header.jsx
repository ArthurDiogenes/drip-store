...
const Header = () => {
  ...
  useEffect(() => {
    // Limpa o campo de busca quando navegar para uma nova página
  ...
  useEffect(() => {
    const handlePopState = () => {
      // Limpa o campo de busca quando navegar com os botões do browser
      setSearchValue('');
    };
  ...
  const handleSearch = (e) => {
    ...
    if (trimmedSearch) {
      ...
      // Cria a URL de busca
      const searchUrl = `/produtos?q=${encodeURIComponent(trimmedSearch)}`;
      
      // Se já estamos na página de produtos, força a navegação
      if (location.pathname === '/produtos') {
        // Atualiza os parâmetros de busca
        const newSearchParams = new URLSearchParams();
        newSearchParams.set('q', trimmedSearch);
        
        // Navega para a nova URL com os parâmetros
        navigate(`/produtos?${newSearchParams.toString()}`, { replace: true });
      } else {
        // Se estamos em outra página, navega normalmente
        navigate(searchUrl);
      }
      
      // Limpa o campo de busca após um pequeno delay
      setTimeout(() => {
        setSearchValue('');
      }, 100);
    }
